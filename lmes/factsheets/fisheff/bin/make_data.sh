...
inFile='lmes_fishing_effort.csv'
template='fisheff_template.html'
temp=''
out="/data/iframes/lmes/factsheets/fisheff"
outddata='data'$temp
...
echo "Fisheries -> Fishing Effort"
echo "==== Starting process ===="
echo "Processing individual data files..."

#Creation of the data files
f=$outdata/$inFile
awk -F ',' '{if (NR > 1){printf "%0d_data.csv %s %s %s\n", $2, $2, $1, $4}}' $f | sort -n | while read fName lmeN year eff
do
	echo $lmeN','$year','$eff >> $outdata/$fName
done
#PROCESSING PACIFIC WARM POOL
f1='/data/public_store/lmes_fisheries/lmes_fishing_effort_pwp.csv'
awk -F ',' '{if (NR > 1){printf "99_data.csv 99 %s %s\n", $1, $2}}' $f1 | while read fName lmeN year eff
do
	echo $lmeN','$year','$eff >> $outdata/$fName
done
...
do
	if [[ $f != *LME* ]]; then
		...
		iframe=${outiframe}/fisheff_${name}.php
