...
<html>
  <head>
	...
	$('#viewData').click(function(){
		var sourceURL = "http://onesharedocean.org/data#250";
		if(sameHost){
		...
		function genChart(LMECode) {
		 ...
		 var options = {
			...
			legend:{align:'center', layout:'horizontal', width:460, itemStyle:{'font-weight': 'normal', 'max-width':'125'}, x:20},
             ...
             yAxis: { title: { text: 'Effective effort (kilowatt)' , useHTML:true}},
             series: [],
...
<?php } ?>
			 tooltip:{valueDecimals: 2, formatter: function() {
				...
				return '<b>LME '+tt+'</b><br/>'+this.series.name+'<br/>' + Highcharts.dateFormat('%Y', new Date(this.x)) +': '+numberWithCommas(this.y.toFixed(2))+' '+legend;
				}
             }};
  ...
  if(plotCounter == 0) {
	...
	var effort_series={name:'Fishing effort', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:plotCounter, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
} else {
  
  
	var effort_series={name:'Fishing effort ('+LMECode+')', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:plotCounter, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
  }
     ...
     $.each(lines, function(lineNo, line) {
		if (line) { // ignore empty line (else lines are not drawn)
          var items = line.split(',');
			items[2] = items[2].replace(/\s/g, '');
			effort_series.data.push( [ Date.UTC(parseInt(items[1]), 5, 15),  parseFloat(items[2]) ] );
		}
	 });
  ...
  if (plotCounter == 0) {
	...
	options.series.push(effort_series);
	
	chart = new Highcharts.Chart(options);
  } else {
	chart.addSeries(effort_series);
