...
<html>
  <head>
	...
	$('#viewData').click(function(){
		var sourceURL = "http://onesharedocean.org/?q=data#245";
		if(sameHost){
		...
		function genChart(LMECode) {
		 ...
		 var options = {
			...
			legend:{align:'center', layout:'horizontal', width:460, itemStyle:{'font-weight': 'normal', 'max-width':'125'}, x:20},
			 ...
			 exporting: {buttons: {contextButton: {symbol: 'url(/sites/all/themes/oceanskeleton/images/download.png)'}}, chartOptions: {title: { text: ''}}},
  ...
  if(iFr == false){
	var catch_series={name:'Catch', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:2, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
  } else {
  ...
  if(iFr == false){
	var catch_series={name:'Catch ('+LMECode+')', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:2, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
  } else {
     ...
     $.each(lines, function(lineNo, line) {
		if (line) { // ignore empty line (else lines are not drawn)
          var items = line.split(',');
				...
				catch_series.data.push( [ Date.UTC(parseInt(items[1]), 06, 15),  parseFloat(items[2]) ] );
  ...
  if (plotCounter == 0) {
		...
		options.series.push(catch_series);
	
	
	chart = new Highcharts.Chart(options);
  } else {
	
		chart.addSeries(catch_series);
 ...
 /*$('.highcharts-contextmenu').click(function(){
	console.log($(this));
 });*/
