...
<html>
  <head>
    ...
    <script type="text/javascript">
	 ...
	 var sourceURL = "http://onesharedocean.org/data#190";
	 if(sameHost){
	 ...
	 var options = {
	   ...
	   legend:{align:'center', layout:'horizontal', width:480, itemStyle:{'font-weight': 'normal', 'max-width':'125'}, x:20},
           title: { text: 'Chlorophyll-A (Canadian High Arctic North Greenland)', x: 0, useHTML: true, align: 'center', style: {font: '14px Verdana, sans-serif', color: '#000000'} },
           xAxis: { type: 'datetime', dateTimeLabelFormats: { year: '%Y'}, title:{enabled:true, text:'Years'}},
           yAxis: { title: { text: 'Chlorophyll-A content (mg.m<sup>-3</sup>)' , useHTML:true}, floor:0 },
           series: [],
	   ...
	   tooltip:{valueDecimals: 2, useHTML: true, formatter: function() {
	     ...
	     if((this.point.series.name.length == 7) || (this.point.series.name.length == 12)){
	       return '<b>LME '+tt+'</b><br/>'+this.point.series.name + '<br/>' + Highcharts.dateFormat('%b/%Y', new Date(this.x)) +': '+this.y.toFixed(2)+' mg.m<sup>-3</sup>';
	     } else {
	       return '<b>LME '+tt+'</b><br/>'+this.point.series.name + '<br/>' + Highcharts.dateFormat('%Y', new Date(this.x)) +': '+this.y.toFixed(2)+' mg.m<sup>-3</sup>';
	     }
	   }
           }};
	 ...
	 if(plotCounter == 0) {
	   ...
	   var yearmonthmean_series={name:'Monthly', data:[], lineWidth:0.5, color: lineColors[plotCounter], zIndex:0, marker:{lineColor: lineColors[plotCounter], lineWidth:0.5, symbol:'cross'}};
	   var yearavg_series={name:'Yearly Average', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:2, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
	   var longterm_series={name:'Long Term Mean', data:[], lineWidth:2,  dashStyle: 'longdash', color: lineColors[plotCounter], zIndex:1, marker:{enabled:false}};
	   
	 } else {
	   
	   var yearmonthmean_series={name:'Monthly ('+LMECode+')', data:[], lineWidth:0.5, color:lineColors[plotCounter], zIndex:0, marker:{lineColor: lineColors[plotCounter], lineWidth:0.5, symbol:'cross'}};
	   var yearavg_series={name:'Yearly Average ('+LMECode+')', data:[], lineWidth:1.5, color: lineColors[plotCounter], zIndex:2, marker:{lineColor: '#ffffff', lineWidth:1.5, fillColor: lineColors[plotCounter], symbol:'circle'}};
	   var longterm_series={name:'Long Term Mean ('+LMECode+')', data:[], lineWidth:2,  dashStyle: 'longdash', color: lineColors[plotCounter], zIndex:1, marker:{enabled:false}};
	 ...
	 $.get(outdata+LMECode+'_data.csv'<?php if($sourceCache == true){?>+'?uid='+rand<?php } ?>, function(data) {
	   ...
	   $.each(lines, function(lineNo, line) {
		 ...
		 case "YM":
		   var temp = null;
		   if(!isNaN(items[3])){
		     temp = parseFloat(items[3]);
		   }
		   yearmonthmean_series.data.push( [ Date.UTC(parseInt(items[1]), parseInt(items[2])-1, 01),  temp ] );
		   break;
		 case "YAVG":
		   yearavg_series.data.push( [ Date.UTC(parseInt(items[1]), 5, 15),  parseFloat(items[2]) ] );
		   break;
		 case "LTAcanceled":
		   longterm_series.data.push( [ Date.UTC(1998, 0, 1), parseFloat(items[1]) ] );
		   longterm_series.data.push( [ Date.UTC(2013, 11, 1), parseFloat(items[1]) ] );
		   break;
		 case 'G':
	   ...
	   if (plotCounter == 0) {
	     options.series.push(yearmonthmean_series);
	     ...
	     chart.addSeries(yearmonthmean_series);
