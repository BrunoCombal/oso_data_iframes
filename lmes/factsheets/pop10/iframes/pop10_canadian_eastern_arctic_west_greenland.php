...
<html>
  <head>
    ...
    <title>LMES Population below 10m</title>
...
$(document).ready(function() {
		...
		var lmesData = [];
		<?php if($zero){ ?>
			var title = "Population Below 10m";
		<?php } else { ?>
			var title = "Population Below 10m (Canadian Eastern Arctic West Greenland)";
		<?php } ?>
...
function genColumn(lmeNumber)
{
			...
			if (line) { // ignore empty line (else lines are not drawn)
				...
				<?php } ?>
					var lme = {
						...
						uKm2: items[4],
						tKm2: items[5],
						rPop2010: parseFloat(items[6]).toFixed(0),
						uPop2010: parseFloat(items[7]).toFixed(0),
						tPop2010: parseFloat(items[8]).toFixed(0),
						rPop2100: parseFloat(items[9]).toFixed(0),
						uPop2100: parseFloat(items[10]).toFixed(0),
						tPop2100: parseFloat(items[11]).toFixed(0),
						nldi: items[12],
						hdi: items[13]
					};
					...
					var den = parseFloat(lme.tPop2010/lme.tKm2).toFixed(3);
					if(den == "NaN"){den = "no data"}
					$(td2).html(den);
					var td3 = document.createElement('TD');
					var res = parseFloat(lme.rPop2010/lme.tPop2010*100).toFixed(0);
					if(res == "NaN"){res = "no data"}else{res = numberWithCommas(lme.rPop2010)+'<br/><span>'+res+'%</spam>';}
					$(td3).html(res);
					var td4 = document.createElement('TD');
					var res = parseFloat(lme.uPop2010/lme.tPop2010*100).toFixed(0);
					if(res == "NaN"){res = "no data"}else{res = numberWithCommas(lme.uPop2010)+'<br/><span>'+res+'%</spam>';}
					$(td4).html(res);
					$(td2).addClass(riskClass(den));
					$(td3).addClass(riskClass(den));
					$(td4).addClass(riskClass(den));
					
					var tdInter = document.createElement('TD');
					
					var td5 = document.createElement('TD');
					var den = parseFloat(lme.tPop2100/lme.tKm2).toFixed(3);
					if(den == "NaN"){den = "no data"}
					$(td5).html(den);
					var td6 = document.createElement('TD');
					var res = parseFloat(lme.rPop2100/lme.tPop2100*100).toFixed(0);
					if(res == "NaN"){res = "no data"}else{res = numberWithCommas(lme.rPop2100)+'<br/><span>'+res+'%</spam>';}
					$(td6).html(res);
					var td7 = document.createElement('TD');
					var res = parseFloat(lme.uPop2100/lme.tPop2100*100).toFixed(0);
					if(res == "NaN"){res = "no data"}else{res = numberWithCommas(lme.uPop2100)+'<br/><span>'+res+'%</spam>';}
					$(td7).html(res);
					$(td5).addClass(riskClass(den));
					$(td6).addClass(riskClass(den));
					$(td7).addClass(riskClass(den));
					
					$('table').append(tr);
					$(tr).append(td1,td2,td3,td4,tdInter,td5,td6,td7);
	...
	function riskClass(item){
	    ...
	    $(function() {
			...
			$('#printPlot')
				.click(function(){
					var text = '/iframes/lmes/factsheets/pop10/iframes/printAll.php';
	...
	<div id="container" style="width:800px; margin:0 auto">
		<table cellspacing="0" cellpadding="0">
			<thead>
				<tr>
					<td></td>
					<td colspan="3" style="border-bottom:1px solid #000">2010</td>
					<td style="width:5px"></td>
					<td colspan="3" style="border-bottom:1px solid #000">2100</td>
					<?php if(!$zero){ ?>
					<td></td>
					<?php } ?>
				</tr>
